...
local _newcclosure = genv.newcclosure or function(f) return f end
...
local function sha256_block(msg, i, H)
    ...
    for j = 17, 64 do
        ...
        w[j] = w[j-16] + bxor(rrotate(v15,7), rrotate(v15,18), rshift(v15,3))
            + w[j-7] + bxor(rrotate(v2,17), rrotate(v2,19), rshift(v2,10))
    end
...
local function sha256_raw(msg)
    ...
    for i = 1, #msg, 64 do sha256_block(msg, i, H) end
    return num2s(H[1],4)..num2s(H[2],4)..num2s(H[3],4)..num2s(H[4],4)
        ..num2s(H[5],4)..num2s(H[6],4)..num2s(H[7],4)..num2s(H[8],4)
end

local _hex_lookup = {}
for i = 0, 255 do _hex_lookup[i] = fmt("%02x", i) end

local function sha256_hex(msg)
    local raw = sha256_raw(msg)
    local t = {}
    for i = 1, 32 do t[i] = _hex_lookup[byte(raw, i)] end
    return concat(t)
end
...
local function b64_encode(data)
    ...
    for i = 1, len - 2, 3 do
        ...
        ti=ti+1; t[ti] = B64E[rshift(n, 18)]
        ti=ti+1; t[ti] = B64E[band(rshift(n, 12), 63)]
        ti=ti+1; t[ti] = B64E[band(rshift(n, 6), 63)]
        ti=ti+1; t[ti] = B64E[band(n, 63)]
    end
    local rem = len % 3
    if rem == 1 then
        local b0 = byte(data, len)
        ti=ti+1; t[ti] = B64E[rshift(b0, 2)]
        ti=ti+1; t[ti] = B64E[lshift(band(b0, 3), 4)]
        ti=ti+1; t[ti] = 61
        ti=ti+1; t[ti] = 61
    elseif rem == 2 then
        local b0, b1 = byte(data, len-1, len)
        local n = b0 * 256 + b1
        ti=ti+1; t[ti] = B64E[rshift(n, 10)]
        ti=ti+1; t[ti] = B64E[band(rshift(n, 4), 63)]
        ti=ti+1; t[ti] = B64E[lshift(band(n, 15), 2)]
        ti=ti+1; t[ti] = 61
    end
    local s = {}
    for i = 1, ti do s[i] = char(t[i]) end
    return concat(s)
end
...
local function hex_encode(d)
    local t = {}
    for i = 1, #d do t[i] = _hex_lookup[byte(d, i)] end
    return concat(t)
end
...
local function sub_word(w)
    return bor(lshift(SBOX[rshift(w,24)+1],24), lshift(SBOX[band(rshift(w,16),0xFF)+1],16),
        lshift(SBOX[band(rshift(w,8),0xFF)+1],8), SBOX[band(w,0xFF)+1])
end
...
local function aes_expand(key)
    ...
    local nr = nk + 6
    local W = {}
    for i = 0, nk-1 do W[i] = s232num(key, i*4+1) end
...
for i = 0, 15 do _aes_s[i]=0; _aes_t[i]=0; _aes_u[i]=0; _aes_m[i]=0 end

local function aes_enc_block(state, W, nr)
    local s = _aes_s
    for i = 0, 15 do s[i] = bxor(state[i+1], band(rshift(W[floor(i/4)], (3-i%4)*8), 0xFF)) end
    for round = 1, nr do
        local t = _aes_t
        for i = 0, 15 do t[i] = SBOX[s[i]+1] end
        local u = _aes_u
        ...
        local rk = round * 4
        for i = 0, 15 do s[i] = bxor(u[i], band(rshift(W[rk+floor(i/4)], (3-i%4)*8), 0xFF)) end
    end
    local out = {}
    for i = 0, 15 do out[i+1] = s[i] end
    return out
end
...
local function aes_dec_block(state, W, nr)
    local s = _aes_s
    local rk = nr * 4
    for i = 0, 15 do s[i] = bxor(state[i+1], band(rshift(W[rk+floor(i/4)], (3-i%4)*8), 0xFF)) end
    for round = nr-1, 0, -1 do
        ...
        local t = _aes_t
        for i = 0, 15 do t[i] = INV_SBOX[u[i]] end
        local rkr = round * 4
        for i = 0, 15 do s[i] = bxor(t[i], band(rshift(W[rkr+floor(i/4)], (3-i%4)*8), 0xFF)) end
        if round > 0 then
            local m = _aes_m
            for col = 0, 3 do
                local c4 = col*4
...
local function pkcs7_pad(d) local p = 16 - (#d%16); return d .. string.rep(char(p), p) end
local function pkcs7_unpad(d) local p = byte(d,#d); if p<1 or p>16 then return d end; return sub(d,1,#d-p) end

local _norm_cache = setmetatable({}, {__mode = "kv"})

local function normalize_key(key)
    if #key == 16 or #key == 24 or #key == 32 then return key end
...
local _block_buf = {}
for i = 1, 16 do _block_buf[i] = 0 end

local function str2block(s, offset)
    local b = _block_buf
    for i = 1, 16 do b[i] = byte(s, offset + i - 1) end
    return b
end
...
local function aes_cbc_enc(plain, key, iv)
    ...
    if not iv or #iv < 16 then
        local ivt = {}
        for i = 1, 16 do ivt[i] = char(random(0,255)) end
        iv = concat(ivt)
    end
    local W, nr = aes_expand(key)
    local padded = pkcs7_pad(plain)
    local prev = {}
    for i = 1, 16 do prev[i] = byte(iv, i) end
    local parts = {iv}
    local pi = 1
    for i = 1, #padded, 16 do
        local block = str2block(padded, i)
        for j = 1, 16 do block[j] = bxor(block[j], prev[j]) end
        prev = aes_enc_block(block, W, nr)
        pi = pi + 1
        local t = {}
        for j = 1, 16 do t[j] = char(prev[j]) end
        parts[pi] = concat(t)
    end
    return concat(parts)
end
...
local function aes_cbc_dec(cipher, key, iv_prepended)
    key = normalize_key(key)
    local iv, data
    if iv_prepended ~= false then iv = sub(cipher,1,16); data = sub(cipher,17)
    else iv = string.rep("\0",16); data = cipher end
    local W, nr = aes_expand(key)
    local prev = {}
    for i = 1, 16 do prev[i] = byte(iv, i) end
    local parts = {}
    ...
    for i = 1, #data, 16 do
        ...
        local raw_block = {}
        for j = 1, 16 do raw_block[j] = block[j] end
        local dec = aes_dec_block(block, W, nr)
        for j = 1, 16 do dec[j] = bxor(dec[j], prev[j]) end
        prev = raw_block
        pi = pi + 1
        local t = {}
        for j = 1, 16 do t[j] = char(dec[j]) end
        parts[pi] = concat(t)
    end
...
local function pbkdf2(pw, salt, iter, klen)
    ...
    for i = 1, floor((klen + 31) / 32) do
        ...
        local r = {}
        for j = 1, #u do r[j] = byte(u, j) end
        for _ = 2, iter do
            u = hmac_sha256(pw, u)
            for j = 1, #u do r[j] = bxor(r[j], byte(u, j)) end
        end
        for j = 1, #r do
            dki = dki + 1
            if dki <= klen then dk[dki] = char(r[j]) end
        end
    end
    return concat(dk)
end
...
function SynSignal:Fire(...)
    ...
    for i = 1, #handlers do
        ...
        if conn and conn.Connected then task.spawn(conn._fn, ...) end
    end
    local waiting = self._waiting
    for i = 1, #waiting do task.spawn(waiting[i], ...) end
    self._waiting = {}
end

function SynSignal:DisconnectAll()
    local handlers = self._handlers
    for i = 1, #handlers do handlers[i].Connected = false end
    self._handlers = {}
    self._waiting = {}
end
...
syn.request = function(params)
    ...
    if not ok then error("syn.request failed: " .. tostring(response), 2) end
    return {
        Success = response.Success or (response.StatusCode and response.StatusCode >= 200 and response.StatusCode < 300) or false,
...
syn.queue_on_teleport = function(script)
    ...
    if _queueteleport then return _queueteleport(script) end
    _teleportQueue[#_teleportQueue+1] = script
end

syn.clear_teleport_queue = function()
    if genv.clear_teleport_queue then return genv.clear_teleport_queue() end
    _teleportQueue = {}
end

syn.get_thread_identity = function()
    if _getidentity then return _getidentity() end
    return 2
end

syn.set_thread_identity = function(id)
    assert(type(id) == "number", "Expected number")
    if _setidentity then return _setidentity(id) end
end

syn.protect_gui = function(gui)
    assert(typeof(gui) == "Instance", "Expected Instance")
    if _gethui then gui.Parent = _gethui()
    else gui.Parent = services.CoreGui end
end
...
syn.ipc_send = function(data)
    if genv.syn_ipc_send then return genv.syn_ipc_send(data) end
end
...
syn.run_on_actor = function(actor, source, ...)
    ...
    if _runonactor then return _runonactor(actor, source, ...) end
...
syn.trampoline_call = function(target, callstack, threadoptions, ...)
    ...
    if genv.trampoline_call then return genv.trampoline_call(target, callstack, threadoptions, ...) end
    local args = {...}
    ...
    if identity_changed then _setidentity(old_identity) end
...
syn.secure_call = function(fn_or_code, scriptInst, ...)
    ...
    if typeof(scriptInst) == "Instance" and _getsenv and _clonefunction and _setfenv then
        ...
        if senv then _setfenv(cloned, senv) end
        return cloned(...)
    end
    return callable(...)
end
...
syn.write_clipboard = function(content)
    if _clipboard then return _clipboard(tostring(content)) end
end

syn.is_beta = function() return false end
...
do
    ...
    syn.oth.hook = function(target, hook)
        ...
        if not _hookfunction then error("hookfunction not available", 2) end
        ...
        local wrapper = _newcclosure(function(...)
            if not hook_entry.active then
                if hook_entry.original then return hook_entry.original(...) end
                return
            end
            ...
            if results[1] then return table.unpack(results, 2)
            else warn("[syn.oth] Hook error: " .. tostring(results[2])) end
        end)

        hook_entry.original = _hookfunction(target, wrapper)
        if not _oth_hooks[target] then _oth_hooks[target] = {} end
    ...
    syn.oth.unhook = function(target, hook_or_callback)
        assert(type(target) == "function", "Expected function")
        local hooks = _oth_hooks[target]
        if not hooks or #hooks == 0 then return false end
        if hook_or_callback then
            for i, entry in ipairs(hooks) do
                if entry.hook == hook_or_callback or entry.original == hook_or_callback then
                    entry.active = false
                    if _restorefunction and #hooks == 1 then pcall(_restorefunction, target) end
                    hooks[i] = hooks[#hooks]; hooks[#hooks] = nil
                    return true
                end
            end
            return false
        ...
        last.active = false
        hooks[#hooks] = nil
        if #hooks == 0 and _restorefunction then pcall(_restorefunction, target); _oth_hooks[target] = nil end
        return true
    end
    ...
    syn.oth.get_root_callback = function()
        if _current_hook_info and _current_hook_info.hook_entry then
            ...
            if hooks and hooks[1] then return hooks[1].original end
        end
        return nil
    end
    ...
    syn.oth.get_original_thread = function()
        if _current_hook_info then return _current_hook_info.original_thread end
        return nil
    end
end
...
syn.crypt.hash = function(algorithm, data)
    if data == nil then data = algorithm; algorithm = "sha256" end
    assert(type(data) == "string")
    local alg = string.lower(algorithm or "sha256")
    if alg == "sha256" or alg == "sha-256" then return sha256_hex(data) end
...
syn.crypt.random = function(size)
    ...
    local t = {}
    for i = 1, size do t[i] = char(random(0,255)) end
    return concat(t)
end

syn.crypt.base64 = {
    encode = function(d) assert(type(d)=="string"); return b64_encode(d) end,
    decode = function(d) assert(type(d)=="string"); return b64_decode(d) end,
}

syn.crypt.hex = {
    encode = function(d) assert(type(d)=="string"); return hex_encode(d) end,
    decode = function(d) assert(type(d)=="string"); return hex_decode(d) end,
}

syn.crypt.url = {
    encode = function(d) assert(type(d)=="string"); return url_encode(d) end,
    decode = function(d) assert(type(d)=="string"); return url_decode(d) end,
}
...
syn.crypt.lz4 = {
    ...
    decompress = function(d, size)
        assert(type(d) == "string")
        if size then return sub(d, 1, size) end
        local s = s232num(d, 1)
        return sub(d, 5, 4 + s)
    end,
}
...
syn.crypt.custom = {
    ...
    hash = function(alg, data) return syn.crypt.hash(alg, data) end,
}

syn.websocket = {
    connect = function(url)
        if _WebSocket and _WebSocket.connect then return _WebSocket.connect(url) end
...
genv.getsynasset = genv.getsynasset or function(path)
    if genv.getcustomasset then return genv.getcustomasset(path) end
